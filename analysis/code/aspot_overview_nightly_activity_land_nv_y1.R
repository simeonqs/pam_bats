...
path_png = 'analysis/results/nightly_activity/nightly_activity_land_nv_y1.png'
...
for(station in sort(unique(dat$station))){
  ...
  for(row in seq_len(nrow(sub_with_detections))){
    if(str_detect(sub_with_detections$species_combined[row], ',')){
      species_found = str_split(sub_with_detections$species_combined[row], ', ')[[1]]
  ...
  sub_with_detections = sub_with_detections[
    !duplicated(sub_with_detections[c('species', 
                                      ...
                                      'night_date')]),]
  sub_with_detections = 
    sub_with_detections[sub_with_detections$species_combined %in% 
                          c('Nnoc', 'Vmur'),]
...
plot.new()
legend('bottom',
       legend = species[species %in% c('Nnoc', 'Vmur')], 
       col = colours[names(colours) %in% c('Nnoc', 'Vmur')], 
       pch = 16,
       cex = 1.5,
       ncol = 3,
       bty = 'n')
