...
# Description: Bayesian version of model for hypothesis 2_2. This version is 
# formulated slightly differently. This version includes day as varying effect.
# >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
...
path_m = 'analysis/code/models/m_h2_2_alternative_day.stan'
path_pdf = 'analysis/results/spatial_model/model_h2_2_alternative_day.pdf'
...
B_year_date = array(0, dim = c(N_years, N_obs, num_knots+2))

for (j in seq_along(unique_years)) {
  year_val = unique_years[j]
  year_rows = which(dat_model$year == year_val)
  
  knots = quantile(dat_model$julian_date[year_rows],
                   probs = seq(0, 1, length.out = 
                                 num_knots))[-c(1, num_knots)]
  
  B = bs(dat_model$julian_date[year_rows],
         knots = knots,
         degree = degree,
         intercept = TRUE)
...
clean_dat = list(N_obs = nrow(dat_model),
                 N_knots = num_knots,
                 N_days = length(unique(dat_model$date)),
                 ...
                 day = dat_model$date |> as.factor() |> as.integer(),
...
trans_subset = c(Buoys = 19,        # circle
                 ...
                 OSS = 15)          # square
pdf(path_pdf, 6, 9)
par(mfrow = c(3, 1), mar = c(4, 4, 0.5, 1))
...
## year 1 ----
subber = dat_model$year[sorter] == 1
plot(dat_model$julian_date[sorter][subber], 
     dat_model$detection[sorter][subber] - 
       rnorm(nrow(dat_model[subber,]), 0.2, 0.05),
     xlim = range(dat_model$julian_date),
     xaxt = 'n', yaxt = 'n',
     pch = trans_subset[dat_model$subset[sorter][subber]], 
     col = c('#c39bd3', # 2023 = purple
             '#f8c471', # 2024 = yellow
             '#d5dbdb'  # 2025 = grey (not included)
     )[dat_model$year[sorter][subber]],
     cex = 0.8,
     xlab = 'Night of year', ylab = 'Probability presence per night')
axis_dates = as.POSIXlt(c('2024-08-01', '2024-08-15', '2024-09-01', 
                          '2024-09-15', '2024-10-01', '2024-10-15'))$yday + 1
...
axis(2, c(0, 0.2, 0.4, 0.6, 0.8))
abline(h = 0, lty = 2, lwd = 2)
nights = dat_model$julian_date[sorter][subber]
year_index = 1
B_plot = B_date[sorter, ][subber, ]                   
B_year_plot = B_year_date[,sorter,][year_index,subber,] 
pred = vapply(seq_along(post$a), function(i)
  vapply(seq_along(nights), function(j)
    post$a[i] +
      B_plot[j, ]        %*% post$w_date[i, ] +
      B_year_plot[j, ]   %*% post$w_year_date[i, year_index, ] -
      post$b[i] * 20,
    numeric(1)),
  numeric(length(nights)))
PI_pred = apply(pred, 1, PI) |> inv_logit()
mean_pred = apply(pred, 1, mean) |> inv_logit()
shade(PI_pred, nights , col = '#e8daef')
lines(nights, mean_pred, lwd = 3, col = '#c39bd3')
## year 2 ----
subber = dat_model$year[sorter] == 2
plot(dat_model$julian_date[sorter][subber], 
     dat_model$detection[sorter][subber] - 
       rnorm(nrow(dat_model[subber,]), 0.2, 0.05),
     xlim = range(dat_model$julian_date),
...
PI_pred = apply(pred, 1, PI) |> inv_logit()
mean_pred = apply(pred, 1, mean) |> inv_logit()
shade(PI_pred, nights , col = '#fcf3cf')
lines(nights, mean_pred, lwd = 3, col = '#f8c471')
dev.off()
