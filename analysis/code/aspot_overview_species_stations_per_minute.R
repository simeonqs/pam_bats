...
path_pdf = 'analysis/results/species_overview/species_overview_per_minute'
...
for(year in c(2023, 2024)){
  ...
  for(sp in species){
    ...
    for(file in files){
      ...
      lines(all_dates, n, col = colours[which(files == file)])
      
    } # end file loop
    
    # Add info plot
    text(as.Date(sprintf('%s-01-05', year)), 0.9*ymax, sp, font = 2, adj = 0)
    if(sp %in% c('M', 'Pnat')){
      axis(2, seq(0, ymax, length.out = 5))
      mtext('Activity minutes', 2, 2.5, cex = 0.75)
    }
