...
warning('Remember to include platform as seperate category.')
...
# Run model
trans_type = c(Buoys = 1L, Windturbines = 2L)
...
for(i in 1:2){
  ...
  lines(rep(i, 2), PIs[,1], lwd = 2)
}
dev.off()

# Run model with also distance to coasat
trans_type = c(Buoys = 1L, Windturbines = 2L)
clean_dat = list(N_obs = nrow(dat_model),
                 N_stations = dat_model$station |> unique() |> length(),
                 present = dat_model$detection |> as.integer(), 
                 ...
                 station = dat_model$station |> as.factor() |> as.integer())
model = cmdstan_model(path_m_station_type_dist_coast)
fit = model$sample(data = clean_dat,
                   seed = 1, 
                   chains = 4, 
                   parallel_chains = 4)
