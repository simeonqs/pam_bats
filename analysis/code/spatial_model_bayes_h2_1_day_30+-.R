...
# Description: Bayesian version of model for hypothesis 2_1.
# This version includes a varying effect for day. 
# This version includes a varying effect for the deployments that had a 
# different schedule. 
# >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
...
path_m = 'analysis/code/models/m_h2_1_day_30+-.stan'
path_png = 'analysis/results/spatial_model/model_h2_1_day_30+-.png'
...
clean_dat = list(N_obs = nrow(dat_model),
                 ...
                 schedule = ifelse(dat_model$date < as.Date('2023-11-01'),
                                   1L, 2L))
...
# Plot predictions ----
## we want the effect of date at 18 degree, 3 m/s and 20 km from the coast
