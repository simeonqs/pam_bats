...
# Description: Bayesian version of model for night of year. Includes varying 
# effect for each day. 
# >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
...
path_m_date_day = 'analysis/code/models/m_date_day.stan'
path_pdf = 'analysis/results/spatial_model/model_date_day.pdf'
...
model = cmdstan_model(path_m_date_day)
