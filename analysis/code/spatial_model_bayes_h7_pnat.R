...
# Description: Bayesian version of model for hypothesis 7. This version in-
# cludes records from only Pnat.
# >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
...
path_png = 'analysis/results/spatial_model/model_h7_pnat.png'
...
plot(dat_model[sorter,]$precip, 
     dat_model[sorter,]$detection_pnat - 
