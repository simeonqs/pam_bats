...
# data. Species abreviations are in the file name. Outputs Aspot training files
...
path_selections = sprintf(
  '%s/aspot_selections/target/TrainingData_Classification_NOVANAbased',
  path_data)
path_wavs = sprintf(
...
export.selection = function(selection, selection_table, wave, file_name){
  ...
  # Create new name 
  type = file_name |> strsplit('_') |> sapply(`[`, 6)
