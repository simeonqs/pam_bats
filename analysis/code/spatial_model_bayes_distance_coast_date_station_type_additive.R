...
# Description: Bayesian version of model for distance to coast, date and 
# station type. Distance to coast and date are allowed to interact. 
# >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
...
path_m_dist_coast_date_station_type_additive = 
  'analysis/code/models/m_dist_coast_date_station_type_additive.stan'
path_pdf = paste0('analysis/results/spatial_model/model_dist_coast_',
                  'date_station_type_additive.pdf')
...
model = cmdstan_model(path_m_dist_coast_date_station_type_additive)
...
pred = vapply(seq_along(post$a), function(i) {
  vapply(seq_along(dists), function(j) {
    post$a[i] + B_plot %*% post$w[i, ] - post$b[i] * dists[j]
  }, numeric(1))
