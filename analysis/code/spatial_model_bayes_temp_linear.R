...
# Description: Bayesian version of model for temperature as linear predictor.
# >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
...
path_m_temp = 'analysis/code/models/m_temp_lin.stan'
path_pdf = 'analysis/results/spatial_model/model_temp_lin.pdf'
...
temps = unique(dat_model$mean_temp[sorter])
pred = vapply(seq_along(post$a), function(i)
  vapply(seq_along(temps), function(j) post$a[i] + post$b[i] * temps[j],
         numeric(1)),
