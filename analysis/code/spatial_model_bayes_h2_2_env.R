...
# Description: Bayesian version of model for hypothesis 2_2.
...
path_m = 'analysis/code/models/m_h2_2.stan'
path_pdf = 'analysis/results/spatial_model/model_h2_2_env.pdf'
...
plot(dat_model$julian_date[sorter], 
     dat_model$detection_env[sorter] - 
...
plot(dat_model$julian_date[sorter][subber], 
     dat_model$detection_env[sorter][subber] - 
...
pred = vapply(seq_along(post$a), function(i)
  vapply(seq_along(nights), function(j) 
    post$a[i] + 
      B_plot[j,] %*% (post$w[i,] + 
                        post$w_year_offset[i,,1] * post$sigma_w[i]) -
      post$b[i] * 20,
    numeric(1)),
  numeric(length(nights)))
PI_pred = apply(pred, 1, PI) |> inv_logit()
...
pred = vapply(seq_along(post$a), function(i)
  vapply(seq_along(nights), function(j) 
    post$a[i] + 
      B_plot[j,] %*% (post$w[i,] + 
                        post$w_year_offset[i,,2] * post$sigma_w[i]) -
      post$b[i] * 20,
    numeric(1)),
  numeric(length(nights)))
PI_pred = apply(pred, 1, PI) |> inv_logit()
