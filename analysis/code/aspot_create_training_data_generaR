...
# clips with the correct file names for Animal Spot. This version uses
# annotations and outputs multiple classes.
# >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
...
# Paths 
path_results = sprintf('aspot/data_sets_g/data_%s/data', data_set)
...
export.selection = function(selection, selection_table, wave, file_name){
  ...
  # Create new name 
  type = selection_table$Annotation[selection]
  type = ifelse(type == 'Mdaub', 'm',
                ifelse(type == 'Mdaub ', 'm',
                       ifelse(type == 'Ppyg', 'p',
                              ifelse(type == 'Nnoc/Eser/Vmur', 'v',
                                     ifelse(type == 'Nnoc', 'v',
                                            type)))))
